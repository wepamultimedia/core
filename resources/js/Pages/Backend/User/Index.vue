<template>
    <MainLayout :bc="[{label: __('users')}]"
        ...
        <div class="flex justify-between my-0 items-center h-14 rounded-lg overflow-hidden my-6">
            <span class="pl-4 dark:text-light font-medium text-xl">{{ __('users_list') }}</span>
            <span class="h-full">
...
const search = _.throttle(value => {
    if (value.length > 0) {
        Inertia.get(route("admin.users.index"), {search: value}, {preserveState: true});
    } else {
        Inertia.get(route("admin.users.index"), {}, {preserveState: true});
    }
}, 100);
