...
class Menu extends Model implements TranslatableContract
{
	 ...
	 * @return array
	 */
	public static function getItems(string $app,
	                                array  $items = null,
		...
		foreach($items as $item) {
			if($item['parent_id'] === $parentId) {
				if(Auth::user()->can($item['can'])) {
					...
					$subItems = self::getItems($app, $items, $item['id']);
	                         ...
	                         int    $parentId = null): void
	{
		if($items) {
			...
			foreach($formatedItems as $item) {
				...
				$newItem['position'] = $this->nextPosition(['parent_id' => $parentId]);
				
				$menu = Menu::create($newItem);
				if(Arr::exists($item, 'position')) {
					$this->updatePosition($menu,
						$item['position'],
						['parent_id' => $parentId]);
				}
